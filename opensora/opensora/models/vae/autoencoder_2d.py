...
class Encoder(nn.Module):
    def __init__(self, config: AutoEncoderConfig):
        ...
        self.conv_in = nn.Conv2d(
            config.in_channels, self.ch, kernel_size=3, stride=1, padding=1
        )
        ...
        self.conv_out = nn.Conv2d(
            block_in, 2 * config.z_channels, kernel_size=3, stride=1, padding=1
        )
...
def AutoEncoderFlux(
    ...
    if from_pretrained:
        model = load_checkpoint(
            model, from_pretrained, cache_dir=cache_dir, device_map=device_map
        )
    return model
